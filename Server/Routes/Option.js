...
router.post("/update", async (req, res) => {
  ...
  if (req.file) {
    ...
    const imageDirPath = path.resolve(filePathpdf.replace(".pdf", ""));
    const oldDirPath = path.resolve(__dirname, "../uploads/old");
    ...
    Emploi_Upload.single("file")(req, res, async () => {
  ToPDF(req.file, req.body.Nom).then(async (_pdfPath) => {
    const imageDirPath = await pdfToImages(_pdfPath);
    ...
    const options = await OptionService.updateOptions(req.body._id, optionData);
    res.send(options);
  });
})}
else{
  ...
  console.log(oldFiliere);
    ...
    const options = await OptionService.insertOption(req.body._id_filiere,optionData);
    res.send(options);
  }
  else{
  const options = await OptionService.updateOption(req.body._id, optionData);
  res.send(options);
  }
